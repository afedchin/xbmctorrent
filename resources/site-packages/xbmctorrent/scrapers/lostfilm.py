...
def lostfilm_index(page=""):
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        try:
            html_data = url_get(urljoin(BASE_URL, "new/page_" + str(page)))
...
def lostfilm_tvshow(alias):
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        fav_btn = str(soup.find_all("div", "favorites-btn2"))
...
def _lostfilm_updateitem_from_db(item, show):
    plugin.log.debug("_lostfilm_updateitem_from_db:" + repr(item) + " == " + repr(show))
...
def _lostfilm_updateitem_from_info(item, result={}):
    if result:
        ...
        poster_url = item.get("info", {}) and base_url + "shmoster_s%s.jpg" % item["info"].get("season", "1")

        result["cover"] = poster_url
        ...
        item.setdefault("properties", {}).update({
            "fanart_image": base_url + "poster.jpg" or "",
        })
...
def _lostfilm_get_meta(show_info):
    plugin.log.debug("_lostfilm_get_meta: " + repr(show_info))
    import re
    from bs4 import BeautifulSoup
    ...
    if details:
        ...
        for line in [line for line in lines if line.strip()]:
            if u"Страна:" in line:
                ...
                info["studio"] = value[:value.find("(")].strip()
