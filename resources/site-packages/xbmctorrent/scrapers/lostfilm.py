...
def url_get(url, params={}, headers={}, post=None):
    ...
    if not _cookiejar:
        ...
        sid_file = os.path.join(CACHE_DIR, 'LostFilm.cookies.dat')
...
def lostfilm_index(page=""):
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        for episode in episodes:
            ...
            season_txt = "%02d" % int(float(season))
            item = _lostfilm_updateitem_from_db({
                "label": "[COLOR FFFFFFCC][%s.%s][/COLOR] [COLOR FFFFFFFF][B]%s[/B][/COLOR]: %s" % (season_txt, episode_num, tvshow_name, episode.text),
            ...
            if "-" not in episode_num and int(episode_num) == 99:
                item.update({
                    "label": "[COLOR FFFFFFCC][%s.xx][/COLOR] [COLOR FFFFFFFF][B]%s[/B][/COLOR]: %s" % (season_txt, tvshow_name, episode.text),
                    "is_playable": False,
                    "info": {
                        "title": episode.text,
                        "season": season,
...
def lostfilm_tvshow(showid, season):
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        if _show_safe_int(showid) in BANNED:
            from xbmctorrent.antizapret import add_host
            add_host(urlparse(BASE_URL).hostname)

        try:
            html_data = url_get(urljoin(BASE_URL, "browse.php"), params={"cat": "_%s" % _show_safe_str(showid)}, headers=HEADERS)
            soup = BeautifulSoup(html_data, "html5lib", from_encoding="windows-1251")
            div_mid = soup.select("div.mid")
        ...
        for episode in episodes:
            ...
            season_txt = "%02d" % int(float(season_num))
            ...
            if is_full_season:
                ...
                label = "[COLOR FFFFFFCC][%s.xx][/COLOR] [COLOR FF00FF00][B]%s[/B][/COLOR]" % (season_txt, episode_name)
            else:
                ...
                label = "[COLOR FFFFFFCC][%s.%s][/COLOR] [COLOR FFFFFFFF][B]%s[/B][/COLOR] %s" % (
                    season_txt, episode_num, nobr.contents[0].text, _safe_list_get(nobr.contents, 2, ""))
...
def lostfilm_play(showid, season, episode):
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        if select >= 0:
            ...
            if (("-" not in episode and int(episode) == 99) or ("." in season and "-" in episode)):
                if "." in season:
...
def _lostfilm_login():
    ...
    login_url = "%s:%s?%s" % (base_parsed.scheme, urljoin(LOGIN_URL, "login.php"), urlencode(params))
    ...
    HEADERS["Origin"] = "%s:%s" % (base_parsed.scheme, LOGIN_URL)
    ...
    if form_action.startswith("//"):
        form_action = "%s:%s" % (base_parsed.scheme, form_action)

    html = url_get(form_action, post=values, headers=HEADERS)
...
def _lostfilm_get_meta(showid):
    ...
    try:
        html = url_get(urljoin(BASE_URL, "browse.php"), params={"cat": "_%s" % _show_safe_str(showid)})
    except:
...
def _lostfilm_dbase():
    ...
    if not _db_connect:
        ...
        db_name = os.path.join(CACHE_DIR, "lostfilm.db")
        first = False
...
def _show_safe_int(showid):
    if showid[0] == "_":
        showid = showid[1:]
    return int(showid)


def _show_safe_str(showid):
    return str(_show_safe_int(showid))
