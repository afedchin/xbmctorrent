...
from xbmctorrent.scrapers import scraper, library
...
def lostfilm_index(page=""):
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        try:
            ...
            _terminate("Не удалось получить данные от сервера")
            return
        ...
        for episode in episodes:
            ...
            item = _lostfilm_updateitem_from_db({
                ...
                "context_menu": [
                    ...
                    ("Добавить в Избранные", actions.background(plugin.url_for("lostfilm_add_favorite", tvid=show["id"])))
                ]
            }, show)
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        for tvshow in tvshows:
            ...
            item = _lostfilm_updateitem_from_db({
                ...
                "context_menu": [
                    ("Добавить в Избранные", actions.background(plugin.url_for("lostfilm_add_favorite", tvid=tvshow["id"])))
                ]
            }, tvshow)
...
def lostfilm_tvshow(alias):
    ...
    with closing(SafeDialogProgress(delay_close=0)) as dialog:
        ...
        for block in serie_blocks:
            ...
            series_table = block.find("table", id=re.compile(r"season_series_(\d+)"))
    ...
    with closing(SafeDialogProgress()) as dialog:
        ...
        try:
            ...
            if html.find("log in first") >= 0:
                ...
                if not plugin.get_setting("lostfilm_login") and not plugin.get_setting("lostfilm_passwd"):
                    _terminate("Проверьте настройки авторизации.", delay=15000)
                    return

                if not _lostfilm_login():
                    _terminate("Авторизация неудалась1. Проверьте настройки.")
                    return
                ...
                if html.find("log in first") >= 0:
                    _terminate("Авторизация неудалась2. Проверьте настройки.")
                    return
        except Exception:
            plugin.log.error("Unexpected error: %s" % sys.exc_info()[0])
            _terminate("Не удалось получить данные от сервера")
        ...
        if select >= 0:
            ...
            if (("-" not in episode and int(episode) == 999) or ("." in season and "-" in episode)):
                ...
                e = re.compile(r"(e[\d]+)?e([\d]+)\.", re.I | re.S | re.U)
...
@plugin.route("/lostfilm/add/<tvid>")
def lostfilm_add_favorite(tvid):

    from json import loads
    try:
        html_data = url_get(BASE_URL + "series/")
        session_id = _lostfilm_get_sid(html_data)
        ...
        params = {"act": "serial", "type": "follow", "id": int(tvid), "session": session_id}
        result = loads(url_get(AJAX_URL, post=params))

        if result.get("result", "") in ("on", "off"):
            plugin.notify("Добавлен в избранное" if result.get("result", "") == "on" else "Удалён из избранного")
            plugin.log.debug("added/removed to/from favorites with result: %s" % result.get("result", ""))
        else:
            plugin.notify("Произошла ошибкаб см. лог")
            plugin.log.error("failed to add/remove to/from favorites with error: %s" % result.get("error", ""))
...
def _terminate(error=None):
    if error:
        import xbmcgui
        xbmcgui.Dialog().notification(plugin.name, error, xbmcgui.NOTIFICATION_ERROR)
    plugin.finish(succeeded=False, cache_to_disc=False)


def _lostfilm_get_sid(html):
    import re
    e = re.search(r"UserData\.session\s*=\s*'([\w\d]+)';", html)
    return e and e.group(1) or ''
...
def _lostfilm_login():
    ...
    if result.get("result", "") == "ok":
        if result.get("need_captcha", False):
            plugin.log.error("LostFilm.tv: login captcha required")
            return False
        if result.get("success", False):
            return True

    plugin.log.error("Login failed with error: %s" % result.get("error", ""))
    return False


def _lostfilm_check_login():
    try:
        from bs4 import BeautifulSoup

        html_data = url_get(BASE_URL + "feedback")
        soup = BeautifulSoup(html_data, "html5lib", from_encoding="utf-8")
        reg_link = soup.select('a[href="/reg"]')

        return False if reg_link else True
    except Exception:
        plugin.log.error("Unexpected error: %s" % sys.exc_info()[0])
        return False
...
@library("LostFilm.tv", True)
@plugin.route("/library/lostfilm/")
def losftfilm_library():
    from json import loads

    if not _lostfilm_check_login() and not _lostfilm_login():
        plugin.log.error("Library integration required authentication, please check settings.")
        return

    if 'kodi-action' in plugin.request.args:
        if plugin.request.args['kodi-action'][0] == 'check_exists':
            plugin.set_resolved_url({})
            return
        elif plugin.request.args['kodi-action'][0] == 'refresh_info':
            pass

    if 'alias' not in plugin.request.args:
        data = []
        tvshows = []

        try:
            params = {"act": "serial", "type": "search", "o": 0, "t": 99, "s": 2}
            while True:
                json_data = url_get(AJAX_URL, params=params)
                data = loads(json_data).get("data", [])
                tvshows += data
        ...
        for item in tvshows:
            ...
            title = "%s [%s]" % (item["title_orig"], item["title"]) if item["title_orig"] else item["title"]
            # Title (Year)
            title = "%s (%s)" % (title, item["date"])

            yield {
                "label": title,
                "path": plugin.url_for("losftfilm_library", alias=item["alias"]),
                "info": {
                    "title": title,
                    "year": item["date"]
                }
        ...
        alias = plugin.request.args['alias'][0]
        try:
            html_data = url_get(BASE_URL + "series/%s/seasons" % alias)
            soup = BeautifulSoup(html_data, "html5lib", from_encoding="utf-8")
            serie_blocks = soup.find_all("div", "serie-block")
        ...
        for block in serie_blocks:
            ...
            if series_table:
                for serie in series_table.find_all("tr"):
                    ...
                    title = "%s [%s]" % (show["title_orig"], show["title"]) if show["title_orig"] else show["title"]
                    play_url = plugin.url_for("lostfilm_play", showid=showid, season=season_num, episode=episode_num)

                    item = {
                        "label": "%s.S%02d.E%02d" % (title, int(season_num), int(episode_num)),
                        "path": plugin.url_for("lostfilm_wrap_play", url=play_url),
                        "is_playable": True,
                        "info": {
                            "title": "%s [%s]" % (title_en.text, title_ru) if title_en and title_en.text else title_ru,
                            "season": season_num,
                            "episode": episode_num
                        }
                    }
...
@plugin.route("/library/lostfilm/play")
def lostfilm_wrap_play():

    if 'kodi_action' in plugin.request.args:
        if plugin.request.args['kodi_action'][0] in ('check_exists', 'refresh_info'):
            plugin.set_resolved_url({})
            return
    if 'url' not in plugin.request.args:
        return

    plugin.redirect(plugin.request.args['url'][0])
